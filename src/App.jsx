...
interface Bil { ar: string; en: string }
...
interface MedRequest {
  ...
  duration: string;
  date: string;
  status: 'Pending' | 'Approved' | 'Rejected' | 'Needs Review' | 'Dispensed' | 'Referred';
  notes: Bil;
  diagnosis?: Bil;
  medType?: 'acute' | 'chronic';
  chronicMonths?: number;
  mealTiming?: 'before' | 'after' | 'with';
}

const SPECIALTIES: Bil[] = [
...
const SEED_REQUESTS: MedRequest[] = [
...
interface Notification { id: string; userId: string; text: Bil; date: string; read: boolean }
interface Alarm { id: string; userId: string; medication: string; time: string; auto?: boolean }
interface CorrectionRequest { id: string; userId: string; userName: string; field: string; reason: string; date: string; status: 'open' | 'resolved' }
interface SupportTicket { id: string; userId: string; userName: string; subject: string; message: string; date: string; status: 'open' | 'resolved' }
interface AuditEntry { id: string; date: string; actorName: string; action: 'created' | 'edited' | 'deactivated' | 'activated' | 'deleted' | 'password_reset'; targetName: string; targetRole: Role }

export default function App() {
  const [isAuthenticated, setIsAuthenticated] = useState(false);
  const [authMode, setAuthMode] = useState<'signin' | 'signup'>('signin');
  const [authRole, setAuthRole] = useState<Role>('patient');
  ...
  const [requests, setRequests] = useState<MedRequest[]>(SEED_REQUESTS);
  const [patientHistory, setPatientHistory] = useState<any[]>(INITIAL_HISTORY);
  const [allergies, setAllergies] = useState<any[]>(INITIAL_ALLERGIES);
  ...
  const [prefs, setPrefs] = useState<{ home: string; work: string; darkMode: boolean }>({ home: '', work: '', darkMode: false });
  ...
  // Patient profile editing state (must be at top — hooks rules)
  ...
  const [adminCreateRole, setAdminCreateRole] = useState<Role>('patient');
  ...
  const [reqMedType, setReqMedType] = useState<'acute' | 'chronic'>('acute');
  const [reqChronicMonths, setReqChronicMonths] = useState('');
  const [reqMealTiming, setReqMealTiming] = useState<'before' | 'after' | 'with'>('after');
  ...
  const logAudit = (action: AuditEntry['action'], target: UserProfile) => {
  ...
  const pushNotification = (userId: string, textAr: string, textEn: string) => {
  ...
  const handleStatusChange = (id: string, newStatus: MedRequest['status']) => {
  ...
  const handleSubmitRequest = (e: FormEvent<HTMLFormElement>) => {
    ...
    let finalMedName: Bil, finalDose: string, finalDuration: string;
  ...
  const handleAddAllergy = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
  ...
  const onMedNameChange = (val: string) => {
    setReqMedName(val);
        ...
        <div className="bg-white p-8 md:p-10 rounded-3xl shadow-xl w-full max-w-md text-center border border-gray-100 fade-in">
          ...
          <div className="mt-8 pt-6 border-t border-gray-100">
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {(['patient', 'doctor', 'pharmacy', 'admin'] as Role[]).map(role => {
          ...
          {currentUser!.role === 'doctor' ? (
            <>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {(['before', 'with', 'after'] as const).map(v => (
  ...
  const submitCorrection = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!currentUser) return;
    const fd = new FormData(e.currentTarget);
  ...
  const handleCreateProfile = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (adminCreateRole === 'admin') {
      ...
      newUser = {
        ...
        dob: String(fd.get('dob') || ''), gender: String(fd.get('gender') || 'male') as 'male' | 'female',
  ...
  const renderManageProfiles = () => (
    <div className="max-w-5xl mx-auto fade-in space-y-6">
      ...
      {(['admin', 'patient', 'doctor', 'pharmacy'] as Role[]).map(role => (
      ...
      {editUser && (
        <Modal onClose={() => setEditUser(null)} title={`${t.edit} — ${getText(editUser.name, lang)}`} t={t}>
          <form onSubmit={(e) => {
            ...
            const fd = new FormData(e.currentTarget);
            const name = String(fd.get('name') || '');
            const updated: UserProfile = { ...editUser, name: { ar: name, en: name } };
            if (editUser.role === 'patient') {
              ...
              updated.gender = String(fd.get('gender') || 'male') as 'male' | 'female';
  ...
  // ---------------- Request Details Modal (preserved) ----------------
...
function EditableField({ label, value, edit, setEdit, editing, type = 'text' }: any) {
  return (
    <div>
      <p className="text-xs text-gray-500 mb-1">{label}</p>
      {editing ? (
...
function Input({ name, label, type = 'text', required, defaultValue }: any) {
  return (
    <div>
...
function Modal({ children, onClose, title, t }: any) {
  return (
...
function HelpModal({ open, onClose, t }: any) {
  if (!open) return null;
  return (
...
function HotlineModal({ open, onClose, t }: any) {
  if (!open) return null;
  return (
